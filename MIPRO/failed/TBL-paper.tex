...
\date{}
\title{Toward Tabular Data Extraction from PDF Documents with a Complex Layout}
...
% in the abstract
\begin{abstract}
PDF is one of the most widespread ways to represent non-editable documents. Many of PDF documents are machine-readable but remain untagged. They have no tags for identifying the logical structure of the text content, e.g., paragraphs, figures, or tables. One of the important challenges here is how to extract tabular data from the documents. The work presents a novel web tool for extracting tables located in untagged PDF documents with a complex layout and recovering their cell structure and content into a structured form (e.g. CSV or HTML). Unlike the existing solutions for PDF table extraction, we suggest using an in-depth document analysis. The proposed tool involves a preprocessing stage that consists in detecting different page items, including page header and footer, sections and subsections, paragraphs, and figures. We also propose two new algorithms for table detection and structure recognition based on recovering a reading order of text in cells accounting a printing order. ICDAR 2013 Table Competition dataset is used to evaluate the prototype of the tool. The F-measure is 83.18\% for the table extraction and 93.64\% for the structure recognition. The results are comparable with the state-of-the-art academician solutions.
\end{abstract}
