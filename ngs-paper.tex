...
\date{}
\title{Rule Based Language and Framework for Spreadsheet Data Transformation}
...
% in the abstract
\begin{abstract}
  A big volume of arbitrary tables (e.g. cross-tabulations, invoices, roadmaps, and data collection forms) circulates in spreadsheet-like formats. These tables can be a valuable data source in business intelligence and data-driven research. However, difficulties that inevitably arise with extraction and integration of the tabular data often hinder the intensive use of them in the mentioned areas. This work is built on the approach to table understanding based on executing rules for table analysis and interpretation with a rule engine. A novel table object model and domain-specific language of table analysis and interpretation rules are defined. In contrast to the existing data transformation languages, we carry on this process as consecutive steps of role analysis, structural analysis, and interpretation. The proposed technique of interpretation is used to transform tabular data into the relational form in terms of the table understanding. The last step of interpretation allows us to convert the spreadsheet data from arbitrary to relational tables. Performance evaluations are carried out automatically for both role and structural analyses with the prepared ground-truth data. A high F-measure from 95.82\% to 99.06\% is shown for different recovered items in the existing dataset of 200 arbitrary tables of the same genre (government statistics).
\end{abstract}
