...
The server is implemented on the base of ClioPatria ontology storage, which in turn is implemented with SWI-Prolog programming system with some C language subroutines.  The ontology database supports durable storage and transactional querying as well as text search facility.  \textbf{ClioPatria implements also the facility for federated SPARQL queries execution out of the box.}  The usage of Prolog based infrastructure for triple processing significantly reduces the structural and semantic complexity of whole system. It supports several formats of compact storage of ontological data.  The Pengines subsystem organizes a regulated access to the triples from clients, it is implemented as a simple protocol on top of HTTP.  There are a number of implementation libraries for the protocols for popular programming languages, including client-side JavaScript, Java, Python.
...
Considered the idea of usage Model Driven Architecture within Semantic Web technologies ....

We implement the transformation process as a set of LogTalk modules....

The approach is successfully used in various application and information systems development ... carcass ...
...
Further development is oriented on further extending the library of LogTalk modules ....
