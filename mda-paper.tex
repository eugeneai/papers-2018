...
The present research deals with developing software instrumental tools for general design of information systems using both Model Driven Architecture (MDA) and LOD \cite{MDA}.  MDA Platform Independent Model (PIM) is expressed as set of UML diagrams (Class Diagram, State Diagram, \emph{etc.}).  The model form a \verb~namespace~ of a LOD graph.  All the PIM entities (MOF structures) are defined as ontology resources, \emph{i.e.} with URI references to LOD terms.  This allows us to translate PIM UML model to set of triples and store them in an ontology warehouse for further transformation into a Platform Specific Model (PSM). The stage of building PIM is very important because it provides a possible future evolution  of the system following the future technology changing. This means that correct and fully described PIM has more value versus PSM.

The task of generation a PSM according on PIM, taking into account the runtime feature and the platform specific, refers to the task of logical inference. Usually one of two approaches is used that are similar to compilation and interpretation of the intermediate code (alike PIM). Applying the library-based approach allows to build of the resulting code enough cheap. Functions from the library may be use for online transform some predicates or implementation of complex steps or subtasks. Also using such interpreted languages or languages with pre-compilations as Python, Perl, PHP, ASP.NET, JavaScript, HTML as a destination language for PSMs allows decrease development time and make it easy.
...
The ontological approach may be used not only to describe the model and data, but also to describe the interface elements, their behavior, the rights and limitations of users and others aspects of specific realisation.
